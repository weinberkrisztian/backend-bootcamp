...
public class ProjectSecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 */
	// using JWT tokens, we do not need CRSF anymore, because the user is validateed by the signature based on the header, body and the secret
	// first we have to override the default behavior of the spring sec, not to generate the JSESSIONID (first row)
	@Override
	protected void configure(HttpSecurity http) throws Exception {
		...
		cors().configurationSource(new CorsConfigurationSource() {
			...
			public CorsConfiguration getCorsConfiguration(HttpServletRequest request) {
				...
				// we must expose the Authorization header (JWT value will be send in that with Bearer scheme)
